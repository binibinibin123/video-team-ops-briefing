...
PROJECT_OVERRIDES = {
    '스냅스': {
        ...
        'priority': '긴급',
        'due': '2026-07-02 오전',
        'bullets': ['표지영 수정 작업 추가.', '수정본 7/2(목) 오전까지 완료 필요.', '원본 6개 제작 완료 이력 유지.', '수정 완료 전 후속/바리에이션 전환 불가.', '헤븐리젤리 원본 8개와 상하목장 2차 소스 업무가 겹쳐 일정 충돌 위험.']
    },
    '비더후드': {'tone': 'risk', 'status': '업로드/비축 · 진행중', 'stage': '업로드/비축', 'bullets': ['정희헌 메인.', '7/2 업로드분까지 비축 완료.', '6/30 한은지 지원 투입 이력.', '7/3 유20 + 7/4 유15 잔여분 비축/예약 확인 필요.', '정희헌은 동아제약 오늘 기획 3개 제작 마무리 예상이라 완료 확인 필요.']},
    '벤슨': {
        'tone': 'focus',
    ...
    '동아제약': {'tone': 'focus', 'status': '높음 · 제작 진행중 · 오늘 3건 마무리 예상', 'stage': '제작 진행중', 'priority': '높음', 'due': '2026-06-30', 'bullets': ['오늘 나온 기획 총 3개.', '정희헌 동아제약 진행중.', '오늘 안에 3개 제작 마무리 예상.', '완료 확정 전까지 산출물/검수 확인 필요.', '비더후드 7/3 이후 잔여분과 병행 일정 체크.']},
    ...
    '헤븐리젤리': {
        ...
        'priority': '긴급',
        'due': '2026-07-02',
        'bullets': ['오늘 신규 프로젝트로 표지영 담당 확정.', '내일 7/2(목)까지 원본영상 8개 산출 필요.', '스냅스 수정 7/2 오전 마감과 동시에 진행.', '상하목장 2차 소스 40개 7/3 오전 마감도 이어져 과부하 위험.', '오늘 오후 제작 가능 시간 역산 및 지원 필요 여부 확인.']
    },
    '한손한끼': {
        'tone': 'waiting',
        'status': '종료 · 운영 제외',
    ...
    '딜로이트(상하목장)': {
        'owner': '표지영',
        'tone': 'risk',
        'status': '긴급 · 소스수급 · 1차 오전마감',
        'stage': '소스수급/폴더링',
        'priority': '긴급',
        'due': '2026-07-01',
        'bullets': ['표지영 담당.', '타입 1~10 총 10개 폴더/영상 소스 수집.', '7/1 오전 1차: 각 4개씩 총 40개.', '7/3 오전 2차: 각 4개 추가, 최종 80개.', '기존 업로드 4개 소스 제외하고 신규 소스 기준.', '정리 경로: X:\\09 프로젝트\\01.영상 바이럴\\딜로이트(상하목장)\\00.소스\\02소스모음']
    },
}
...
PEOPLE_OVERRIDES = {
    ...
    '표지영': '헤븐리젤리 신규 프로젝트 원본영상 8개 7/2까지 제작. 스냅스 수정 7/2 오전까지 완료 필요. 상하목장은 1차 완료 확인 후 2차 7/3 오전 추가 40개. 쿠쿠홈시스는 고객사 소스 재전달 이슈로 보류. 표지영 과부하 주의.',
    '정희헌': '비더후드 7/2 업로드분까지 비축 완료. 현재 동아제약 오늘 기획 총 3개 진행중이며 오늘 안에 3개 제작 마무리 예상. 완료 확정 전까지 산출물/검수 확인 필요. 7/3 이후 비더후드 잔여분 확인 필요.',
...
def build():
    ...
    key_changes = [
        {'label': '헤븐리젤리', 'title': '신규 프로젝트 · 표지영 담당 · 원본 8개', 'body': '헤븐리젤리 신규 프로젝트 표지영 담당 확정. 2026-07-02까지 원본영상 8개 산출 필요. 스냅스 수정/상하목장 소스 업무와 일정 충돌 위험.'},
        {'label': '스냅스', 'title': '수정 작업 추가 · 7/2 오전 마감', 'body': '스냅스는 원본 6개 제작 완료 이력 유지. 새 수정 작업은 표지영 담당으로 2026-07-02 오전까지 완료 필요. 수정 완료 전 후속/바리에이션 전환 불가.'},
        {'label': '상하목장', 'title': '표지영 소스 수집 · 7/1 오전 1차', 'body': '딜로이트(상하목장) 타입 1~10 소스 폴더링. 7/1 오전 1차 각 4개씩 총 40개, 7/3 오전 2차 추가 40개. 기존 업로드 4개 소스 제외.'},
        ...
        {'label': '동아제약', 'title': '오늘 기획 총 3개 · 당일 제작 마무리 예상', 'body': '동아제약 오늘 나온 기획은 총 3개. 정희헌이 진행중이며 오늘 안에 3개 제작 마무리 예상. 완료 확정 전 산출물/검수 확인 필요.'},
    ...
    recent_updates = [
        ...
        {'heading': '2026-06-30 23:39 — 스냅스 원본 완료/상하목장 누락 정정', 'text': '스냅스 원본 6개 제작 완료 이력. 이후 7/1 수정 요청 반영으로 현재는 7/2 오전까지 수정 진행중. 딜로이트(상하목장) 표지영 소스 수집/폴더링 건도 장부와 앱에 반영.'},
        ...
        {'heading': '2026-06-30 17:16 — 동아제약 오늘 기획 3개', 'text': '동아제약 오늘 나온 기획은 총 3개. 정희헌은 동아제약 진행중이며 오늘 안에 3개 제작 마무리 예상. 완료 확정 전 산출물/검수 확인 필요.'},
        {'heading': '2026-06-30 15:09 — 비더후드 비축/동아제약 진행', 'text': '비더후드는 7/2 업로드분까지 비축 완료. 동아제약은 17:16 정정 기준 오늘 기획 총 3개 진행중.'},
    ...
    data = {
        'meta': {
            'title': '영상팀 운영상황판',
            'lastUpdated': '2026-07-01 14:22 KST · 헤븐리젤리 신규 원본 8개 / 스냅스 수정 오전마감 반영',
