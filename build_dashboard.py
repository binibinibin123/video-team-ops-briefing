...
PROJECT_OVERRIDES = {
    ...
    '삼양': {
        'owner': '조성주',
        'tone': 'risk',
        'status': '긴급 · 7/19 종료예정 · 60만 부족',
        'stage': '성과회수/추가바리검토',
        'priority': '긴급',
        'due': '2026-07-19 종료예정',
        'bullets': ['탱글 종료일 7/19 예정 공유.', '목표 조회수까지 약 60만 회 부족.', '5~10번 영상 제작+바리에이션 발행 가능성.', '막판 몰아 발행 리스크가 있어 김영훈이 중간 빈 일정 분산 추가 발행 가능 여부 문의.', '최종 운영 방향/일별 슬롯/바리 개수/컨펌 범위 확정 전.', '조성주는 미닉스 바리에이션과 병행해야 해 부하 주의.']
    },
    '미닉스': {
        'tone': 'risk',
...
PEOPLE_OVERRIDES = {
    ...
    '조성주': '미닉스 7/4까지 바리에이션 36개와 삼양 탱글 7/19 종료 전 추가 운영을 병행해야 해 부하 높음. 삼양은 목표 조회수 약 60만 회 부족, 5~10번 영상 제작+바리 가능성, 중간 빈 일정 분산 발행 여부 확인 필요.',
}
...
def build():
    ...
    key_changes = [
        {'label': '삼양/탱글', 'title': '7/19 종료예정 · 60만 부족 · 분산발행 검토', 'body': '탱글 종료일이 7/19로 잡힐 가능성이 높고 목표 조회수까지 약 60만 회 부족. 5~10번 영상 제작+바리에이션 발행 가능성이 있어 일정이 빠듯하며, 김영훈이 막판 몰아 발행 대신 중간 빈 일정 분산 발행 가능 여부를 문의한 상태.'},
    ...
    recent_updates = [
        {'heading': '2026-07-02 18:41 — 삼양(탱글) 종료일/추가 바리 예고', 'text': '탱글 종료일 7/19 예정, 목표 조회수 약 60만 회 부족. 5~10번 제작+바리 가능성이 있어 김영훈이 중간 빈 일정 분산 발행 가능 여부를 문의한 상태. 확정 전이라 일별 물량은 아직 미추가.'},
    ...
    data = {
        'meta': {
            'title': '영상팀 운영상황판',
            'lastUpdated': '2026-07-02 18:41 KST · 삼양 탱글 7/19 종료/60만 부족 반영',
