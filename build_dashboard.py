...
PROJECT_OVERRIDES = {
    '스냅스': {
        ...
        'bullets': ['7/2(목) 오전 메인 수정본 전달 예정.', '목요일 중 컨펌 완료 시 7/7(화)~7/12(일) 총 60건 발행 예정.', '컨펌 완료 시점에 따라 발행 시작일 지연 가능.', '일별 배분은 미정이라 우선 주간 총량 60건으로 관리.', '표지영은 헤븐리젤리 원본 8개/상하목장 2차와 겹쳐 과부하 위험.']
    },
...
PEOPLE_OVERRIDES = {
    ...
    '표지영': '스냅스 메인 수정본 7/2 오전 전달 후 목요일 중 컨펌 추적. 컨펌 완료 시 7/7(화)~7/12(일) 총 60건 발행 예정. 헤븐리젤리 원본영상 8개 7/2까지 제작, 상하목장 2차 7/3 오전 추가 40개, 쿠쿠홈시스는 소스 재전달 이슈 보류. 표지영 과부하 주의.',
...
def build():
    ...
    data = {
        'meta': {
            'title': '영상팀 운영상황판',
            'lastUpdated': '2026-07-01 15:45 KST · 스냅스 7/2 수정본 전달/차주 60건 조건 반영',
